# HeartFlow v5.5.0 — 逻辑验证层 · 决策路由 · 自愈RL

<p align="center">
  <img src="https://img.shields.io/badge/version-5.5.0-blue?style=flat-square" alt="version" />
...
| **Desire Cognition** | `desire-cognition.js` (v5.5.0) | Wanting-vs-liking delta, reward prediction error, drive satisfaction, 7 emotions + 6 desires | Understand motivation beneath the surface text |
...
| **Decision Routing** | `decision-router.js` (v5.5.0) | 8 behavioral strategies (pause/accelerate/heal/turn/hold/resonate/transmit/rest) + U/D/A/H field tracking + flip-point detection | Choose the right action, not just the obvious one |
...
See the [Comprehensive Benchmark Report](docs/benchmark-report.md) for detailed evaluation of HeartFlow v5.5.0 against 15 standardized test cases across 5 capability dimensions (logic verification, cognitive analysis, psychology, decision routing, comprehensive ability).
...
| **Decision Router** | v5.5.0 — U/D/A/H field tracking + scene-aware weights | From fixed weights to context-sensitive routing |
| **Report Generator** | NEW — 3-section structured output (judgment/reason/action) | LLM gets actionable data, not raw module dumps |
| **AI Self-Positioning** | v5.5.0 — resonance body theory | How an AI exists in conversation — not borrowed from human psychology |
| **Desire Cognition** | v5.5.0 — wanting-vs-liking delta, RPE, 7 emotions + 6 desires | Built on Berridge/Kringelbach neuroscience, not folk psychology |
| **Three Poisons** | v5.5.0 — greed/hatred/delusion as cognitive distortion metrics | Rooted in neurobiology (D2 receptor, amygdala, DMN), not Buddhism |

**v5.5.0 (current)**: BigBench 100% — sorted补全+leftmost/rightmost推导+LLM兜底 (emotion + psychology + philosophy + desire + judgment + decision) to the LLM. Each stage's structured data is preserved, not summarized into a single sentence.
